...
- (BHBlockTableInfo*)didSelectSectionRow:(NSInteger)row {
    ...
    self.info.deselectRowAutomatically = YES;
...
- (BHBlockTableInfo*)didDeselectSectionRow:(NSInteger)row {
...
- (void)willDisplayCell:(id)cell forSectionRow:(NSInteger)row {
    [self setupInfoForRow:row];
    self.info.cell = cell;
...
- (void)configureSectionHeaderView:(id)headerView {
    ...
    if (self.configureHeaderView && !self.hasConfiguredHeader) {
